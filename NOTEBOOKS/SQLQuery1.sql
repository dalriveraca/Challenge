...
with t0 as (select [user_id],[event_data value_prop] from [Challenge].[dbo].[taps]
...
t2 as (
select [user_id],[event_data value_prop],COUNT(*) as conteo_print from [Challenge].[dbo].[prints]
where [day] < '{{FECHA_SEMANAS}}'--2020-11-23
group by [user_id],[event_data value_prop]
--order by 1
...
t3 as (
select [user_id],[event_data value_prop],COUNT(*) as conteo_tap from [Challenge].[dbo].[taps]
where [day] < '{{FECHA_SEMANAS}}'--2020-11-23
group by [user_id],[event_data value_prop]
--order by 1
),
